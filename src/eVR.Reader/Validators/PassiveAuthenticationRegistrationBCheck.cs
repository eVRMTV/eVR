...
{
    /// <summary>
    /// Class used to check the hashes over EF Registration_B in EF SOD and in EF Signature_B
    /// </summary>
    /// <param name="csCaCache">A cache with the CSCA certificates</param>
    /// <param name="logger">A logger</param>
    public class PassiveAuthenticationRegistrationBCheck(
          CsCaCache csCaCache
        , ILogger<PassiveAuthenticationRegistrationBCheck> logger)
        : IValidator
    {
        #region Properties

        public string Name => "Passive Authentication Registration B";
        ...
        /// <summary>
        /// Validate whether the hashes over EF Registration_B in EF SOD and in 
        /// EF Signature_B are correct
        /// </summary>
        ...
        public async Task<bool> Validate(eVRCardState state)
        {
            ...
            {
                var ds = state.C_IA_B_DS.DSCertificate!;
        ...
        private bool CheckEfSodHash(eVRCardState state)
        {
            ...
            var hashedEF = hashAlgoritmSod.ComputeHash(state.RegistrationB.RawData);
            var hashEF = state.SOd.DatagroupHashValues[state.RegistrationB.FileIDHexValue];
            if (!Helper.CompareByteArrays(hashedEF, hashEF))
            {
                logger.LogError("Could not verify the hash of EF Registration B in EF SOD");
                return false;
            }
            return true;
        }
        ...
        {
            var descipheredSignature = Helper.Decrypt(ds, state.SignatureB.Signature).Reverse().Take(32).Reverse().ToArray();
            using var hashAlgoritmSignature = Oids.GetHashAlgorithm(state.SignatureB.SignatureAlgorithmOid!);
            var hashedRegistration = hashAlgoritmSignature.ComputeHash(state.RegistrationB.RawData);
            var result = Helper.CompareByteArrays(descipheredSignature, hashedRegistration);
            if (result)
            {
                logger.LogInformation("Passive Authentication Registration B Check finished succesfully.");
            }
            else
            {
                logger.LogError("Could not verify Signature B.");
            }
            return result;
        }
        #endregion
